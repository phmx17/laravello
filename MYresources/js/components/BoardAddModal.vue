...
import UserBoards from '../graphql/UserBoards.gql'; // in ordre to update cache for it
...
export default {
  ...
  methods: {
    addBoard() {
      const self = this;
      this.$apollo.mutate({
        mutation: BoardAdd, // imported file name
        variables: {
          title: this.title,
          color: this.color
        },
        update(store, {data: { boardAdd }}) {   // boardAdd = defined name of mutation in BoardAdd.gql
