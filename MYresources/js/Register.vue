  ...
  <div class="bg-white sm:bg-gray-100 h-full flex justify-center" >
    <div class="container mt-2 sm:mt-10 flex flex-col items-center">
      ...
      <div class="w-full sm:shadow-xl sm:bg-white sm:py-8 sm:px-12">
        ...
        <div class="w-full text-center text-gray-600 font-bold mb-8">Register New Account</div>
        <form @submit.prevent="register">
          ...
          <div class="w-full mb-6">
            <button type="submit" class="rounded-md px-4 py-2 text-sm bg-green-500 font-bold outline-none focus:outline-none hover:bg-opacity-75 w-full text-white disabled:opacity-25">
              Sign Up
            </button>
          </div>
        </form>
  ...
  methods: {
    async register() {
      ...
      try {
        ...
        if (user) {
          this.$store.dispatch('setLoggedIn', true);  // dispatch flag
          this.$store.commit('setUser', user); // commit user data to store
          this.$router.push({ name: "board" });   // redirect to board
        }      
      } catch(err) {
        this.errors = gqlErrors(err); // call our nifty custom error tool
      }
    }
  }
}
